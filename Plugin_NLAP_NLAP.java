////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                            //
//                                        GAME IMAGER                                         //
//                              Game Image Manipulation Utility                               //
//                                http://www.watto.org/imager                                 //
//                                                                                            //
//                           Copyright (C) 2006-2009  WATTO Studios                           //
//                                                                                            //
// This program is free software; you can redistribute it and/or modify it under the terms of //
// the GNU General Public License published by the Free Software Foundation; either version 2 //
// of the License, or (at your option) any later versions. This program is distributed in the //
// hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranties //
// of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License //
// at http://www.gnu.org for more details. For updates and information about this program, go //
// to the WATTO Studios website at http://www.watto.org or email watto@watto.org . Thanks! :) //
//                                                                                            //
////////////////////////////////////////////////////////////////////////////////////////////////

import org.watto.Language;
import org.watto.component.WSProgressDialog;
import org.watto.manipulator.*;

import java.io.File;

import javax.swing.ImageIcon;
import java.awt.*;
import java.awt.image.*;
import java.io.*;


/**
**********************************************************************************************

**********************************************************************************************
**/
public class Plugin_NLAP_NLAP extends ArchivePlugin {


/**
**********************************************************************************************

**********************************************************************************************
**/
  public Plugin_NLAP_NLAP() {
    super("NLAP_NLAP","NLAP_NLAP");

    //            read write
    setProperties(true,true);
    setIsCollection(false);

    setGames("Hitman: Codename 47");
    setExtensions("nlap"); // MUST BE LOWER CASE
    setPlatforms("PC");
    }


/**
**********************************************************************************************

**********************************************************************************************
**/
  public int getMatchRating(FileManipulator fm) {
    try {

      int rating = 0;

      if (fm.getExtension().equals(extensions[0])){
        rating += 25;
        }

      // 4 - Header
      if (fm.readString(4).equals("NLAP")){
        rating += 50;
        }
      else {
        rating = 0;
        }

      // 4 - File Length
      if (fm.readIntL() == fm.length()){
        rating += 5;
        }

      fm.skip(4);

      // 2 - Image Height
      if (check.height(fm.readShortL())){
        rating += 5;
        }

      // 2 - Image Width
      if (check.width(fm.readShortL())){
        rating += 5;
        }

      return rating;

      }
    catch (Throwable t){
      return 0;
      }
    }


/**
**********************************************************************************************

**********************************************************************************************
**/
  public Resource[] read(File path) {
    try {

      FileManipulator fm = new FileManipulator(path,"r");

      long arcSize = fm.length();

      // 4 - Header (NLAP)
      // 4 - File Length (including all these header fields)
      fm.skip(8);

      // 4 - File ID
      int fileID = fm.readIntL();

      // 2 - Image Height
      short height = fm.readShortL();
      check.height(height);

      // 2 - Image Width
      short width = fm.readShortL();
      check.width(width);


      // 4 - Number Of Mipmaps
      int numMipmaps = fm.readIntL();

      // 4 - File Type (20)
      fm.skip(4);

      // 4 - Hash?
      int hash = fm.readIntL();

      // X - Filename
      // 1 - null Filename Terminator
      String filename = fm.readNullString();


      // 4 - Data Length
      fm.skip(4);


      // X - Pixels
      int numPixels = width*height;
      int[] pixels = new int[numPixels];
      for (int p=0;p<numPixels;p++){
        // 1 - Color Palette Index
        pixels[p] = fm.readByteU();
        }

      // skip the other mipmaps
      for (int i=1;i<numMipmaps;i++){
        // 4 - Data Length
        int length = fm.readIntL();
        check.length(length,arcSize);

        // X - Pixels
        fm.skip(length);
        }


      // 4 - Number Of Colors
      int numColors = fm.readIntL();

      // X - Palette
      int[] palette = new int[numColors];
      for (int i=0;i<numColors;i++){
        // 1 - Red
        // 1 - Green
        // 1 - Blue
        // 1 - Alpha
        int b = fm.readByteU();
        int g = fm.readByteU();
        int r = fm.readByteU();
        int a = fm.readByteU();

        palette[i] = ((a << 24) | (b << 16) | (g << 8) | (r));
        }


      fm.close();

      Resource resource = new Resource(path,pixels,width,height,palette);

      resource.addProperty("DDSNumMipMaps",""+numMipmaps);
      resource.setProperty("Hash",""+hash);
      resource.setProperty("FileID",""+fileID);
      resource.setProperty("Filename",filename);

      return new Resource[]{resource};
      }
    catch (Throwable t){
      logError(t);
      return null;
      }
    }


/**
**********************************************************************************************
Writes an [archive] File with the contents of the Resources
**********************************************************************************************
**/
  public void write(Resource[] resources, File path) {
    try {

      FileManipulator fm = new FileManipulator(path,"rw");

      Resource resource = resources[0];

      WSProgressDialog.setMessage(Language.get("Progress_WritingFiles"));



      int width = resource.getWidth();
      int height = resource.getHeight();



      int numMipMaps = 1;
      String mipMapCount = resource.getProperty("DDSNumMipMaps");
      try {
        numMipMaps = Integer.parseInt(mipMapCount);
        }
      catch (Throwable t){
        }

      int fileID = 0;
      String fileIDString = resource.getProperty("FileID");
      try {
        fileID = Integer.parseInt(fileIDString);
        }
      catch (Throwable t){
        }

      int hash = 0;
      String hashString = resource.getProperty("Hash");
      try {
        hash = Integer.parseInt(hashString);
        }
      catch (Throwable t){
        }

      String filename = resource.getProperty("Filename");



      // 4 - Header (NLAP)
      fm.writeString("NLAP");

      // 4 - File Length (including all these header fields)
      fm.writeIntL(0);

      // 4 - File ID
      fm.writeIntL(fileID);

      // 2 - Image Height
      fm.writeShortL((short)height);

      // 2 - Image Width
      fm.writeShortL((short)width);

      // 4 - Number Of Mipmaps
      fm.writeIntL(numMipMaps);

      // 4 - File Type? (20)
      fm.writeIntL(20);

      // 4 - Hash?
      fm.writeIntL(hash);

      // X - Filename
      // 1 - null Filename Terminator
      fm.writeString(filename);
      fm.writeByte(0);


      // Reduce to 256 colors, if necessary
      ColorConverter.changeNumColors(resource,256);


      int[] pixels = resource.getPixels();

      // X - Mipmaps
      for (int i=0;i<numMipMaps;i++){

        int numPixels = pixels.length;

        // 4 - Data Length
        fm.writeIntL(numPixels);

        // X - Pixels
        for (int p=0;p<numPixels;p++){
          // 1 - Color Palette Index
          fm.writeByte((byte)pixels[p]);
          }

        // prepare the next mipmap
        if (i != numMipMaps-1){
          int newWidth = width/2;
          int newHeight = height/2;

          int[] newPixels = new int[newWidth*newHeight];
          for (int h=0,j=0;h<height;h+=2,j++){
            for (int w=0,x=0;w<width;w+=2,x++){
              //// average of the 4 pixels
              //newPixels[j*newWidth+x] = (pixels[h*width+w]+pixels[h*width+w+1]+pixels[h+1*width+w]+pixels[h+1*width+w+1])/4;
              newPixels[j*newWidth+x] = pixels[h*width+w];
              // Left top pixel

              }
            }

          pixels = newPixels;
          width = newWidth;
          height = newHeight;
          }

        }





      int[] palette = resource.getPalette();
      int numColors = palette.length;

      // 4 - Number Of Colors
      fm.writeIntL(numColors);


      // X - Palette
      for (int i=0;i<numColors;i++){
        int color = palette[i];

        // 1 - Blue
        // 1 - Green
        // 1 - Red
        // 1 - Alpha
        fm.writeByte(color>>16);
        fm.writeByte(color>>8);
        fm.writeByte(color);
        fm.writeByte(color>>24);
        }


      // go back and write the file length
      fm.seek(4);
      fm.writeIntL((int)fm.length());


      fm.close();

      }
    catch (Throwable t){
      logError(t);
      }
    }


  }