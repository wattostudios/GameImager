...
**/
  public void loadArchiveTable(){
    ...
    if (Archive.getReadPlugin() != null){
      ...
      ArchivePlugin plugin = null;
      Resource selected = ((WSFileListPanelHolder)WSRepository.get("FileListPanelHolder")).getCurrentResource();
      if (selected != null){
        plugin = selected.getReadPlugin();
        }

      if (plugin == null){
        plugin = Archive.getReadPlugin();
        }
...
**/
  public void loadResourceTable(Resource[] resources){
    ...
    if (resources.length == 1){
      tableData = new String[][]{{Language.get("Information_Width"),resources[0].getWidth() + " " + Language.get("Information_Pixels")},
                                 {Language.get("Information_Height"),""+resources[0].getHeight() + " " + Language.get("Information_Pixels")},
                                 {Language.get("Information_NumColors"),""+resources[0].getNumColors()}
                                 };
      }
    else {
      tableData = new String[][]{{Language.get("Information_NumFiles"),resources.length + " "}};
      }
  ...
  public boolean onDoubleClick(JComponent c, MouseEvent e){
    if (c instanceof WSTable){
      ...
      if (code.equals("SidePanel_Information_PluginTable")){
        ...
        Resource selected = ((WSFileListPanelHolder)WSRepository.get("FileListPanelHolder")).getCurrentResource();
        Task_ReadWithDifferentPlugin task = new Task_ReadWithDifferentPlugin(selected.getSource(),plugin,selected);
...
**/
  public void reloadTable(){
    ...
    WSFileListPanelHolder fileListHolder = ((WSFileListPanelHolder)WSRepository.get("FileListPanelHolder"));
    ...
    Resource selectedFile = fileListHolder.getCurrentResource();
    if (selectedFile != null){
      loadPluginTable(selectedFile.getSource());
      }
    else {
      loadEmptyTable(pluginTable);
      }

    WSPanel fileListPanel = fileListHolder.getCurrentPanel();
    if (fileListPanel == null || !(fileListPanel instanceof FileListPanel)){
      loadEmptyTable(resourceTable);
      }
    else {
      Resource[] resources = fileListHolder.getSelected();
      if (resources == null || resources.length == 0){
        loadEmptyTable(resourceTable);
        }
      else {
        loadResourceTable(resources);
        }
      }
    }
