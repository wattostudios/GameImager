...
**/
  public Plugin_MAP() {
    ...
    setGames("Baram");
    setExtensions("map"); // MUST BE LOWER CASE
    setPlatforms("PC");
    }
...
**/
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      // 2 - Image Width
      if (check.width(fm.readShortB())){
        rating += 5;
        }

      // 2 - Image Height
      if (check.height(fm.readShortB())){
        rating += 5;
        }
...
**/
  public Resource[] read(File path) {
    try {
      ...
      // 2 - Width
      short width = fm.readShortB();
      check.width(width);

      // 2 - Height
      short height = fm.readShortB();
      check.height(height);
...
**/
  public void write(Resource[] resources, File path) {
    try {
      ...
      // 2 - Width
      fm.writeShortB((short)resource.getWidth());

      // 2 - Height
      fm.writeShortB((short)resource.getHeight());
